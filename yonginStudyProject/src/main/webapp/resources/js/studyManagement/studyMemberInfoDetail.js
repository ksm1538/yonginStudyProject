/** 변수 설정(시작) **/  
var studyMemberInfoDetailBinder = new ax5.ui.binder();	// Binder 설정(데이터를 받아오면 이 Binder set 함으로써 데이터 자동으로 들어감)
...
$(document).ready(function () {
	...
	studyMemberInfoDetailBinder.setModel({}, $(document["studyMemberInfoDetailForm"]));
	
	selectStudyMemberInfoDetail(parentData.userCode);
...
// 해당 멤버 정보 조회
function selectStudyMemberInfoDetail(userCode){
	...
	$.ajax({
 		type: "POST",
 		url : "/studyManagement/selectStudyMemberInfoDetail.json",
 		data : userCode,
