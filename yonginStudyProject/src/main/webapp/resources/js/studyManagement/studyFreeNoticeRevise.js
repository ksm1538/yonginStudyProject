...
$(document).ready(function () {
	...
	studyFreeNoticeInfoDetailBinder.setModel({}, $(document["reviseNoticeForm"]));
	
	reviseStudyFreeNotice(parentData.boardCode);
	
});
/** 초기화(끝) **/

// 부모 페이지에서 받은 boardCode를 이용해 스터디 자유게시판 조회
function reviseStudyFreeNotice(boardCode){
	if(boardCode == ""){
	...
	$.ajax({
 		type: "POST",
 		url : "/studyManagement/selectStudyFreeNoticeRevise.json",
 		data : boardCode,
...
//스터디 자유게시판 수정 함수
function reviseStudyFreeNoticeFunc(){
    ...
    $.ajax({
        ...
        url: "/studyManagement/studyFreeNoticeRevise",
        data: data,
