...
$(document).ready(function () {
	...
	studyNoticeInfoDetailBinder.setModel({}, $(document["reviseStudyNoticeForm"]));
...
function reviseStudyNotice(boardCode){
	if(boardCode == ""){
	...
	$.ajax({
 		type: "POST",
 		url : "/studyManagement/selectReviseStudyNotice.json",
 		data : boardCode,
...
//공지사항 수정 함수
function reviseStudyNoticeFunc(){
	
	// 폼 데이터 가져옴
    var form = $('#reviseStudyNoticeForm')[0];
    var data = new FormData(form);

    $.ajax({
        type: "POST",
        enctype: 'multipart/form-data',
        url: "/studyManagement/selectReviseStudyNotice",
        data: data,
