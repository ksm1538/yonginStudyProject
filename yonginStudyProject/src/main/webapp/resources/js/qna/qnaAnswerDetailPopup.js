...
$(document).ready(function () {
	...
	qnaBinder.setModel({}, $(document["qnaAnswerDetailForm"]));
...
// 부모 페이지에서 받은 boardCode를 이용해 qna 답글 조회
function selectqnaDetail(boardCode){
	...
	$.ajax({
 		type: "POST",
 		url : "/systemQna/selectQnaAnswerDetail.json",
 		data : boardCode,
		...
		success : function(data, status, xhr) {
			switch(data.result){
			    case COMMON_SUCCESS:
			    	...
			    	if(data.boardInfo.qnaStatus == "" || data.boardInfo.hgrnkBoardCode != ""){
			    		$("#answerBtn").hide();
			    	}
			    	...
			    	getReplyList(data.boardInfo.boardCode);	//	댓글 조회
...
// 수정하기 버튼 함수
function openReviseQnaAnswer(){
	location.href = "/systemQna/reviseQnaAnswer.do";
}

// 해당 QnA 삭제
function deleteQnaAnswer(){
	dialog.confirm({
		msg:"해당 답글을 삭제하시겠습니까?",
		btns:{
			yes: {
				label:'네', onClick:function(key){
					...
					var sendData = {
							...
							hgrnkBoardCode : parentData.boardCode
					}
					
					$.ajax({
				 		type: "POST",
				 		url : "/systemQna/deleteQnaAnswer.json",
...
function saveReply(){
	var sendData = {
			boardCode : qnaBinder.get("boardCode"),
